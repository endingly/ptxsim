...
constexpr auto pack(bool negative, unsigned exponent,
  ...
  return T{static_cast<Bits>(sign |
...
template <typename T>
Result<T> convert_nan(Fp32 value) noexcept {
  using Traits = FormatTraits<T>;
...
Fp32 widen_nan(From value, ExceptionFlags& flags) noexcept {
  ...
  return Fp32{static_cast<std::uint32_t>((negative ? 0x80000000u : 0) |
                                         0x7F800000u | payload)};
}
...
NormalizedBf16 unpack_bf16(Bf16 value) noexcept {
  using Traits = FormatTraits<Bf16>;
  constexpr unsigned fraction_lsb = std::countr_zero(Traits::fraction_mask);
  constexpr std::uint32_t hidden =
      std::uint32_t{1} << Traits::fraction_bits;
  const bool negative = is_negative(value);
...
Result<Bf16> round_pack_bf16(bool negative, int exponent,
  ...
  if (exponent < minimum_exponent) {
    significand_with_grs = shift_right_jam(
  ...
  if (exponent > maximum_exponent) {
    ...
    return {finite ? maximum_finite<Bf16>(negative)
                   : infinity<Bf16>(negative),
            flags};
  }
  ...
  return {pack<Bf16>(negative, exponent_field, fraction), flags};
}

bool finite_bf16(Bf16 value) noexcept {
  const auto category = classify(value);
  return category == FpClass::Zero || category == FpClass::Subnormal ||
         category == FpClass::Normal;
}

Result<Bf16> special_bf16_binary(Bf16 lhs, Bf16 rhs, ArithmeticControl control,
                                 Operation operation) {
  if (operation == Operation::Add && is_infinity(lhs) && is_infinity(rhs) &&
      is_negative(lhs) != is_negative(rhs))
    return canonical_invalid_nan<Bf16>();
  if (operation == Operation::Mul &&
      ((is_infinity(lhs) && is_zero(rhs)) ||
  ...
  Result<Fp32> intermediate;
  switch (operation) {
    case Operation::Add:
      intermediate = SoftFloatBackend<Fp32>::add(a, b, {control.rounding});
      break;
    case Operation::Mul:
      intermediate = SoftFloatBackend<Fp32>::mul(a, b, {control.rounding});
      break;
    default:
      std::unreachable();
  }
  auto result = narrow_from_f32<Bf16>(intermediate.value, {control.rounding});
...
Result<Bf16> add_finite_bf16(Bf16 lhs, Bf16 rhs,
  ...
  if (is_zero(lhs) && is_zero(rhs)) {
    ...
    return {Bf16{static_cast<std::uint16_t>(
                negative ? FormatTraits<Bf16>::sign_mask : 0)},
            {}};
  }
  if (is_zero(lhs))
    return {rhs, {}};
  ...
  if (a.negative == b.negative) {
    ...
    constexpr std::uint32_t carry =
        (std::uint32_t{1} << (FormatTraits<Bf16>::fraction_bits + 1)) << 3;
    if (result_sig >= carry) {
    ...
    if (result_sig == 0)
      return {Bf16{static_cast<std::uint16_t>(
                  rounding == RoundingMode::TowardNegative
                      ? FormatTraits<Bf16>::sign_mask
                      : 0)},
              {}};
    constexpr std::uint32_t hidden =
        (std::uint32_t{1} << FormatTraits<Bf16>::fraction_bits) << 3;
...
struct ExactBf16Integer {
  ...
  [[nodiscard]] bool any_below(unsigned end) const noexcept {
    ...
    return remaining != 0 && (words[whole_words] &
                              ((std::uint64_t{1} << remaining) - 1)) != 0;
  }
};

Result<Bf16> round_exact_bf16(const ExactBf16Integer& value, int lsb_exponent,
                              bool negative, RoundingMode mode) noexcept {
...
Result<To> narrow_from_f32(Fp32 value, ConversionControl control) {
  static_assert(ConversionTraits<To, Fp32>::supported);
  validate_conversion_control(control);
  using Traits = FormatTraits<To>;
  const bool negative = is_negative(value);
  const auto category = classify(value);
  if (category == FpClass::QuietNaN || category == FpClass::SignalingNaN)
    return convert_nan<To>(value);
  if (category == FpClass::Infinity)
    return Traits::has_infinity && !control.satfinite
               ? Result<To>{infinity<To>(negative), {}}
               : overflow<To>(negative, control);
  if (category == FpClass::Zero)
...
template <typename From>
Result<Fp32> widen_to_f32(From raw, ConversionControl control) {
  static_assert(ConversionTraits<Fp32, From>::supported);
  ...
  if (exponent_field == 0 && fraction == 0)
    return {Fp32{negative ? 0x80000000u : 0u}, {}};
  ...
  return {
      Fp32{static_cast<std::uint32_t>(
          (negative ? 0x80000000u : 0) |
          (static_cast<std::uint32_t>(exponent + 127) << 23) | f32_fraction)},
      {}};
}

Result<Bf16> Bf16Backend::add(Bf16 lhs, Bf16 rhs, ArithmeticControl control) {
...
Result<Bf16> Bf16Backend::sub(Bf16 lhs, Bf16 rhs, ArithmeticControl control) {
  validate_control<Operation::Sub, Bf16>(control);
  if (is_nan(lhs) || is_nan(rhs))
    return propagate_nan(lhs, rhs);
  rhs.bits ^= FormatTraits<Bf16>::sign_mask;
  if (!finite_bf16(lhs) || !finite_bf16(rhs))
    return special_bf16_binary(lhs, rhs, control, Operation::Add);
  return add_finite_bf16(lhs, rhs, control.rounding);
}
Result<Bf16> Bf16Backend::mul(Bf16 lhs, Bf16 rhs, ArithmeticControl control) {
  validate_control<Operation::Mul, Bf16>(control);
  if (is_nan(lhs) || is_nan(rhs))
    return propagate_nan(lhs, rhs);
  if (!finite_bf16(lhs) || !finite_bf16(rhs))
...
Result<Bf16> Bf16Backend::fma(Bf16 a, Bf16 b, Bf16 c,
                              ArithmeticControl control) {
  validate_control<Operation::Fma, Bf16>(control);
  ...
  if (is_infinity(a) || is_infinity(b)) {
    if (is_infinity(c) && is_negative(c) != product_negative)
      return canonical_invalid_nan<Bf16>();
    return {infinity<Bf16>(product_negative), {}};
  }
  if (is_infinity(c))
    return {c, {}};
  ...
  if (product_zero && is_zero(c)) {
    const bool negative = product_negative == is_negative(c)
                              ? product_negative
  ...
  const std::uint32_t product =
      unpacked_a.significand * unpacked_b.significand;
...
template Result<Bf16> narrow_from_f32(Fp32, ConversionControl);
template Result<Tf32> narrow_from_f32(Fp32, ConversionControl);
template Result<Fp8E4M3> narrow_from_f32(Fp32, ConversionControl);
template Result<Fp8E5M2> narrow_from_f32(Fp32, ConversionControl);
template Result<Fp4E2M1> narrow_from_f32(Fp32, ConversionControl);
template Result<Fp32> widen_to_f32(Bf16, ConversionControl);
template Result<Fp32> widen_to_f32(Tf32, ConversionControl);
template Result<Fp32> widen_to_f32(Fp8E4M3, ConversionControl);
template Result<Fp32> widen_to_f32(Fp8E5M2, ConversionControl);
template Result<Fp32> widen_to_f32(Fp4E2M1, ConversionControl);
