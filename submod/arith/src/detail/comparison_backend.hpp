...
[[nodiscard]] constexpr T absolute(T value) noexcept {
  ...
  return T::from_bits(static_cast<Bits>(normalize_encoding(value).bits() &
...
[[nodiscard]] Result<T> minmax_impl(T lhs, T rhs, MinMaxControl minmax_control,
  ...
  if (relation == 0 && is_zero(compared_lhs) && is_zero(compared_rhs)) {
    result = T::from_bits(static_cast<typename FormatTraits<T>::Bits>(
...
[[nodiscard]] inline Result<float32_t> minmax(float32_t a, float32_t b, float32_t c,
                                         MinMaxControl minmax_control,
                                         ArithmeticControl control,
                                         bool choose_min) {
  validate_minmax_modifiers<float32_t>(minmax_control, true);
  if (choose_min)
    validate_control<Operation::Min, float32_t>(control);
  else
    validate_control<Operation::Max, float32_t>(control);
...
[[nodiscard]] Result<T> copysign(T sign, T magnitude) {
  ...
  const auto magnitude_bits = normalize_encoding(magnitude).bits() &
                              static_cast<Bits>(~FormatTraits<T>::sign_mask);
  return {T::from_bits(static_cast<Bits>(sign_bits | magnitude_bits)), {}};
}
...
[[nodiscard]] Result<bool> testp(T value, TestpOp operation) {
  ...
  switch (operation) {
    ...
    case TestpOp::Infinite:
      return {category == fp_class::infinity, {}};
    case TestpOp::Number:
      return {!is_nan(value), {}};
    case TestpOp::NotANumber:
      return {is_nan(value), {}};
    case TestpOp::Normal:
      return {category == fp_class::zero || category == fp_class::normal, {}};
    case TestpOp::Subnormal:
      return {category == fp_class::subnormal, {}};
  }
  std::unreachable();
}
