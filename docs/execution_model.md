...
is an optional `frontend-lowering` feature-on build-tree diagnostic target; it
is not installed or exported. Only that target privately links `ptx_frontend`;
`arith` remains independent. No PTX program is lowered or executed yet.
...
remain future work. There is no lowering context/frontend-ID map, module or
instruction lowering, or lowering install/export surface yet.
